...
| POA-2 | Conduct first contingency plan tabletop exercise | Medium | CP-4 | CEO / ISSO | Q2 2026 | Remediated — Automated monthly CP-4 test via `contingency-test` CLI + GitHub Actions cron (1st of month). Checks: Cloud SQL backup/PITR, GCS versioning, Cloud Run health, KMS keys, Artifact Registry. Reports archived to GCS evidence bucket and accessible via the 3PAO assessment portal. |
...
| POA-5 | Complete first annual incident response exercise | Medium | IR-3 | CEO / ISSO | Q2 2026 | Remediated — Monthly IR-3 red team exercise via GitHub Actions cron (1st of month). 99 automated attacks across 6 suites (auth bypass, escalation, exfiltration, leftfield, webapp, manual tools). MITRE ATT&CK mapped. Reports archived to GCS evidence bucket and accessible via the 3PAO assessment portal. |
