...
func templatedCAControls() []ControlDef {
	return []ControlDef{
		...
		{ID: "ca-3", ImplStatus: "implemented", RoleID: "system-owner", Baseline: "moderate", NarrativeFn: func(f *InfraFacts) string { return "System interconnections are authorized, documented, and managed via Terragrunt: (1) Cross-project IAM — app SA receives cloudsql.client, storage.objectViewer, and aiplatform.user on the ops project; admin SA receives cloudtasks.enqueuer, storage.objectAdmin, and cloudsql.client on the ops project; Cloud Build SAs receive firebase.viewer and identityplatform.viewer on auth projects for deployment validation; (2) Cross-project Pub/Sub — Cloud Scheduler publishes cron events to ops project via push subscriptions; (3) Customer IdP connections via SAML/SCIM require explicit admin configuration; (4) Cloudflare-to-origin connections secured via Cloud Armor Cloudflare-only restriction; (5) GitHub-to-GCP connections via Workload Identity Federation." }},
...
func templatedCMControls() []ControlDef {
	return []ControlDef{
		...
		{ID: "cm-3", ImplStatus: "implemented", RoleID: "system-owner", Baseline: "moderate", ComponentUUIDs: []string{thisSystem, ciCD},
			NarrativeFn: func(f *InfraFacts) string {
				deployDesc := "Cloud Build deployment with digest-pinned container images"
				if f.CloudBuildBinauthzEnabled {
					deployDesc = "Cloud Build deployment with container signing (Binary Authorization)"
				}
				return fmt.Sprintf("Configuration changes are controlled through: (1) Git-based PR workflow requiring review and approval; (2) Terragrunt plan output reviewed before apply; (3) FedRAMP SCN classification on all PRs (GitHub Actions); (4) CI/CD pipeline validation (lint, test, security scan) before merge; (5) %s.", deployDesc)
			}},
...
func templatedPMControls() []ControlDef {
	return []ControlDef{
		...
		{ID: "pm-3", ImplStatus: "implemented", RoleID: "system-owner", Baseline: "moderate", NarrativeFn: func(f *InfraFacts) string { return "Security and privacy resources are allocated through: (1) dedicated compliance infrastructure (compliance repo, automated tools); (2) engineering time for security controls; (3) third-party security assessment budget; (4) security tooling budget (scanning tools, compliance automation)." }},
		{ID: "pm-4", ImplStatus: "implemented", RoleID: "system-owner", Baseline: "moderate", NarrativeFn: func(f *InfraFacts) string { return "POA&M is maintained in the SSP (Appendix I) and updated continuously as vulnerabilities are discovered, assessed, and remediated. POA&M items include milestones and completion dates. The 3PAO assessment portal provides real-time POA&M visibility." }},
...
func templatedSIControls() []ControlDef {
	return []ControlDef{
		...
		{ID: "si-2", ImplStatus: "implemented", RoleID: "system-owner", Baseline: "moderate", NarrativeFn: func(f *InfraFacts) string { return "Flaw remediation: (1) critical vulnerabilities patched within 24 hours; (2) high within 7 days; (3) medium within 30 days; (4) low within 90 days. Vulnerability scanning runs daily via Cloud Build. Patch status tracked in POA&M." }},
		{ID: "si-2.2", ImplStatus: "implemented", RoleID: "system-owner", Baseline: "moderate", NarrativeFn: func(f *InfraFacts) string { return "Automated flaw remediation status monitoring: (1) daily vulnerability scans (GoSec, Semgrep, Trivy, govulncheck) report results to Cloud Build; (2) scan results archived to GCS evidence bucket; (3) Cloud Monitoring alerts on new critical/high findings." }},
		...
		{ID: "si-4.4", ImplStatus: "implemented", RoleID: "system-owner", Baseline: "moderate",
			NarrativeFn: func(f *InfraFacts) string {
				flowLogDetail := "VPC flow logs capture network-level traffic metadata"
				if f.FlowLogSampling > 0 {
					flowLogDetail = fmt.Sprintf("VPC flow logs capture network-level traffic metadata (%.0f%% sampling)", f.FlowLogSampling*100)
				}
				return fmt.Sprintf("Inbound and outbound traffic monitoring: (1) Cloudflare provides full visibility into inbound HTTP traffic (requests, responses, WAF actions, threat scores); (2) %s; (3) Cloud Armor logs capture origin-layer WAF decisions; (4) VPC egress firewall logs capture outbound connection attempts.", flowLogDetail)
			}},
