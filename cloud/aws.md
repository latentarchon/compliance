...
|---|---|---|---|
| App (staging) | Staging | `latentarchon-app` | User-facing API, SPA, OpenSearch, Bedrock |
| Admin (staging) | Staging | `latentarchon-admin` | Admin API, ops, RDS, S3, SQS, Textract, Comprehend |
...
|---|---|---|---|---|
| DB access | App ECS task role | Admin account | RDS IAM auth + cross-account assume role | Read-only database access from app API |
...
| `archon-ops` | Admin | `SERVER_MODE=ops` | Background processing (embeddings, DLP, cron) | 1–5 tasks |
| `admin-spa` | Admin | nginx | React SPA for administrators | 1–5 tasks |
| `clamav` | Admin | ClamAV REST | Internal malware scanning service | 1–2 tasks |
...
|---|---|---|
| **IAM SAML Providers** | Both | Per-tenant SAML IdP registration (Okta, Entra ID, etc.) |
| **WAFv2** | Both | OWASP managed rules, rate limiting, IP allowlisting |
| **ALB** | Both | TLS termination (ACM certs), host-based routing |
| **SQS** | Admin | Async document processing queue (+ dead letter queue) |
| **Textract** | Admin | Document text extraction (OCR) |
| **Comprehend** | Admin | PII detection in uploaded documents |
| **CloudWatch** | Both | Logs, metrics, alarms, dashboards |
| **CloudTrail** | Both | API call logging and audit trail |
| **ECR** | Both | Docker image storage with vulnerability scanning |
| **Route 53** | App | DNS records + ACM certificate validation |
| **ACM** | Both | TLS certificates for ALB |
...
|---|---|---|
| `archon-app-task` | App API | RDS IAM auth (read-only), OpenSearch access, Bedrock invoke |
| `archon-admin-task` | Admin API | RDS IAM auth (read-write), S3 (CRUD), SQS (send) |
| `archon-ops-task` | Ops service | RDS IAM auth (ops scope), S3 (CRUD), SQS (receive), Textract, Comprehend, Bedrock |
...
| Attribute | Admin Account | App Account |
|---|---|---|
| **CIDR** | `10.10.0.0/16` | `10.11.0.0/16` |
| **AZs** | 3 (`us-east-1a/b/c`) | 3 (`us-east-1a/b/c`) |
| **Subnets** | Public (ALB), private (ECS), database (RDS) | Public (ALB), private (ECS) |
| **NAT Gateway** | Enabled (one per AZ for HA) | Enabled (one per AZ) |
| **VPC Endpoints** | S3, SQS, KMS, Textract, Comprehend, ECR, CloudWatch | OpenSearch, Bedrock, ECR, CloudWatch |
| **Flow Logs** | Enabled → CloudWatch Logs | Enabled → CloudWatch Logs |
...
1. **AWS Organizations** with two accounts (app + admin) in the correct OU
